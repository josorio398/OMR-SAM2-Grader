def run_pipeline_exact():
    ...
    # ---------------------------------------------------------
    dir_antes = "debug_antes"
    dir_despues = "debug_despues"
    os.makedirs(dir_antes, exist_ok=True)
    os.makedirs(dir_despues, exist_ok=True)
    ...
    # ---------------------------------------------------------
    print("Sube el archivo PDF con las hojas de respuestas...")
    ...
    for num_pagina in range(total_paginas):
        ...
        try:
            ...
            cv2.imwrite(f"{dir_antes}/pagina_{num_pagina + 1}.jpg", image_cv)
            ...
            # --- SAM 2 ---
            sam_results = sam_model.predict(source=image_cv_rgb, bboxes=cajas_para_sam, save=False, verbose=False)
            ...
            cv2.imwrite(f"{dir_despues}/pagina_{num_pagina + 1}.jpg", viz_image)
            ...
            resultados_ocr, _ = ocr_engine(tercio_superior_raw)
    ...
    # ---------------------------------------------------------
    # 4. EXPORTACIÓN FINAL A EXCEL Y DESCARGA DE DIAGNÓSTICO
    # ---------------------------------------------------------
    print("\n" + "="*50)
    print("📊 CONSOLIDANDO RESULTADOS Y EXPORTANDO...")
    df_resultados_finales = pd.DataFrame(datos_salon)

    # display() solo existe en notebook; en Colab funciona.
    try:
        from IPython.display import display
        display(df_resultados_finales)
    except Exception:
        print(df_resultados_finales)
    ...
    print(f"✅ ¡Archivo '{nombre_archivo_salida}' descargado!")

    print("📦 Comprimiendo imágenes de diagnóstico...")
    shutil.make_archive("Imagenes_Diagnostico", 'zip', ".", base_dir="debug_antes")
    os.system("zip -ur Imagenes_Diagnostico.zip debug_despues")
    files.download("Imagenes_Diagnostico.zip")
    print("✅ ¡Archivo 'Imagenes_Diagnostico.zip' descargado!")

    return {
        "excel": nombre_archivo_salida,
        "zip": "Imagenes_Diagnostico.zip",
        "df": df_resultados_finales,
    }
